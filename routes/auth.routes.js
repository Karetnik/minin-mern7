  ...
  try {
    ...
    if (!errors.isEmpty()) {
      return res.status(400).json({
        errors: errors.array(),
        message: 'Ошибка данных при регистрации'
      })
    }
    ...
    if (candidate) {
      return res.status(400).json({message: 'Пользователь с указанным email существует'})
    }
...
router.post('/login',
  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({message: 'Пользователь с указанными email отсутствует'})
    }
